...
func main() {
	...
	if err := migrations.UpP_4(db.GetDB(), logger); err != nil {
		logger.Fatal(err, logging.Postgres, logging.Migration, err.Error(), nil)
	}
	if err := migrations.UpP_5(db.GetDB(), logger); err != nil {
		logger.Fatal(err, logging.Postgres, logging.Migration, err.Error(), nil)
	}
	if err := migrations.UpP_6(db.GetDB(), logger); err != nil {
		logger.Fatal(err, logging.Postgres, logging.Migration, err.Error(), nil)
	}
	...
	customToolWorker := jobqueue.NewWorker(jobStore, logger, custom_tools.CustomToolQueue)
	custom_tools.RegisterCustomToolWorker(customToolWorker, sshconnService, customToolService, toolDeployer)
	go func() {
